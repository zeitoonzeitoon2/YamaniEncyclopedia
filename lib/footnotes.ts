export function applyFootnotes(input: string, locale: string = 'ar', externalDefinitions?: Record<string, string>): string {
  ...
  const items = Object.keys(refOrder)
    ...
    .map((id) => {
      ...
      return `<li id="fn:${id}" class="hover:bg-amber-900/5 p-1 rounded transition-colors"><span class="text-amber-600 dark:text-amber-400 font-semibold me-2">${n}.</span>${defHtml} <a href="#fnref:${id}" class="text-amber-600 dark:text-amber-400 no-underline hover:underline ml-1" aria-label="رجوع">↩︎</a></li>`
    })
    .join('')
...
<div class="mt-12 pt-6 border-t border-amber-700/30">
  <h3 class="text-xl font-bold mb-4 text-amber-600 dark:text-amber-400">${footnotesTitle}</h3>
  ...
  if (headings.length) {
    const items = headings
      .map((h) => {
        ...
        return `<li class="mb-1"><a href="#h-${h.id}" class="text-amber-700 dark:text-amber-300 hover:text-amber-500 hover:underline underline-offset-2 ${size}" style="margin-right:${mr}px">${escapeHtml(h.text)}</a></li>`
      })
      .join('')
    ...
    const title = `<div class="mb-2 flex flex-row-reverse items-center justify-between"><span style="color:var(--site-accent);font-size:1.35rem;font-weight:bold">${tocTitle}</span>${controls}</div><div class="mt-1" style="border-top:1px solid var(--site-border);opacity:.6"></div>`
